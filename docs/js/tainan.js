...
function pointStyleFunction(f) {
  var p = f.getProperties(), color, stroke, radius, typeText;
  if (f === currentFeature) {
...
function showPoint(pointId) {
  ...
  for (k in features) {
    ...
    if (p.id === pointId) {
      ...
      var code = p.id.split('-')[0];
      var message = '<table class="table table-dark">';
      message += '<tbody>';
      message += '<tr><th scope="row" style="width: 100px;">名稱</th><td>';
      message += '<a href="http://www.nhi.gov.tw/QueryN/Query3_Detail.aspx?HospID=' + code + '" target="_blank">' + p.name + '</a>';
      message += '</td></tr>';
      if (p.line && p.line !== '') {
        message += '<tr><th scope="row">LINE ID</th><td>' + p.line.replace(/\\n/g, '<br />') + '</td></tr>';
      }
...
var adminTree = {};
var findTerms = [];
$.getJSON('json/tainan.json', {}, function (c) {
  pointsFc = c;
